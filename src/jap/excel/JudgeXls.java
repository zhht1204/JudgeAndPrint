...
public class JudgeXls implements JudgeDocument {
    private File file;
    ...
    public boolean[] isColored() {
        if(this.OPENOFFICE_HOME != null) {
            DocumentConverter ptpc = new DocumentConverter(this.OPENOFFICE_HOME);
            File tempFolder = new File(System.getProperty("java.io.tmpdir") + "japTemp");
            if (!tempFolder.exists() || !tempFolder.isDirectory()) {
                tempFolder.mkdir();
            }
            ptpc.converter(file, new File(tempFolder.getPath() + "/temp.pdf"));
            Pdf2JpgConverter conv = new Pdf2JpgConverter();
            conv.convertPdf2Jpg(tempFolder.getPath() + "/temp.pdf");
